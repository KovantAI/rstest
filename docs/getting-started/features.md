...
!!! tip "New here? Start with three"
    ...
    pytest? `rstest try` gives the verdict in one command (it runs your suite
    under serial pytest, then under rstest). The full surface:

| Feature | Flag / API | Notes |
|---|---|---|
| Parallel by default | `-n auto` (default) | logical cores, capped by test-file count and cached suite time; `-n 0` = byte-exact pytest session |
...
| Flaky handling | `--reruns N`, `@pytest.mark.flaky`, `--only-rerun` | failed-then-passed = flaky (green run, counted, listed); crash-aware. Works at any `-n`: at `-n 0/1`, `--reruns` runs a one-worker pool (not byte-exact) |
...
*is* pytest underneath. In parallel, session fixtures run once per worker, as
under pytest-xdist. See [Compatibility](../concepts/compatibility.md).
