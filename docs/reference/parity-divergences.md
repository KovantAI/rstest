...
## 9. Order-dependent serial baseline
...
serial-versus-parallel artifact in the baseline, not an rstest or
follower-database defect.

**Upstream fix:** make these tests independent of full-suite order, so the
serial run no longer skips them.
