...
(pytest-mypy, the report aggregators, corpus-loaded plugins) also runtime-`V`.
...
| 66 | pytest-pretty | 1.2M | 🔶 `-n 0` | V | Rich terminal painting; rstest owns the terminal at `-n ≥ 2` (benign, corpus-loaded). |
...
| 80 | pytest-anyio | 809K | ✅ Works | V | anyio async tests per worker (corpus-loaded). |
...
| 95 | pytest-examples | 595K | ✅ Works | V | Code-example / docstring testing (corpus-loaded). |
...
next verification tranche (see `BACKLOG.md`).

## Summary by verdict (ranks 51–100)

- **✅ Works — 22:** vcr, flakefinder, datadir, testinfra, flask, freezegun, freezer, alembic, opentelemetry, qt, describe, sftpserver, regressions, ansible, race, doctestplus, mpl, variables, examples, mypy, subprocess, flake8.
...
- **⚠️ Caveat — 7:** docker, celery, docker-tools, reportportal, pytestrail, csv (+ the 🔴/🟦 azurepipelines).
- **🔴 Silent — 4:** reportlog, md, nunit, azurepipelines (single-master aggregators — emit at `-n 0` or use a native artifact).
- **➖ N/A — 8:** lazy-fixtures, cases, assume, cache, clarity, lazy-fixture, pylint-pytest, describe-adjacent format/assertion helpers.

The fault line is unchanged from the top-50: a plugin that reads
...
or paints the terminal (pretty, profiling). None are broken; the master-side
...
unlisted plugins) is designed in `warn-dead-master-path-plan.md` but **not yet
implemented**. Two false-positive shapes it will have to handle, from analyzing
...
- **sqlalchemy** test conftest — a real `workerinput["follower_ident"]`
  subscript, yet it runs crash-free at `-n 4` because rstest **seeds**
  `follower_ident` via its `pytest_configure_node` emulation; a static scan
  would need to model that runtime seeding.

Both refinements are tracked in `BACKLOG.md` §"Dead-master-path detector
precision". No genuinely non-compatible plugin has surfaced across the corpus.
