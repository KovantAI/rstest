...
!!! tip "Pin for reproducible CI"
    ...
    pin a version (`pip install rstest==0.5.0` or `rstest~=0.3`) or install
    from your lockfile.
...
jobs:
  tests:
    ...
    steps:
      ...
      - uses: actions/setup-python@v5
        with: { python-version: "3.13" }
      - run: pip install -r requirements.txt && pip install rstest==0.5.0
