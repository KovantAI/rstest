...
and a `pytest<9` install pin has no effect at runtime. rstest warns about
such a pin: one `rstest: warning: <plugin> <version> requires pytest<9, ...`
line to stderr per run (Unreleased: not in 0.7.0). See
